...
示例:
  ...
  python main.py --closure-repair # 在安全窗口内尝试补齐闭环缺口
    ...
    parser.add_argument("--backfill-universe", choices=["pool", "all", "active"], default=None, help="回填股票池: pool=研究池 all=全部A股 active=活跃股，默认pool")
    parser.add_argument("--backfill-full", action="store_true", help="全区间重拉并修复中间缺口（默认只补尾部增量）")
    parser.add_argument("--backfill-start", default=None, help="全区间起点 YYYYMMDD，默认20160101")
    parser.add_argument("--backfill-workers", type=int, default=None, help="回填并发数，默认按CPU/内存自适应")
    ...
    parser.add_argument("--task-prefix", default="AlphaPilot", help="Windows计划任务名前缀")
    parser.add_argument("--service-prefix", default="alpha-pilot", help="Linux systemd用户任务名前缀")
