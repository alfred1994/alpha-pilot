...
def main():
    ...
    try:
        ...
        assert_true('source .venv/bin/activate' in run_auto, "自动盘脚本兼容README推荐.venv")
        assert_true("export BROKER_MODE=paper" in run_auto, "自动盘脚本默认模拟盘")
        assert_true("main.py --auto" in run_auto, "自动盘脚本执行--auto")
        assert_true("data/auto_trader.lock" in run_auto, "自动盘脚本清理残留锁")
        assert_true("export BROKER_MODE=paper" in restart_auto, "自动盘重启脚本默认模拟盘")
        assert_true('source venv/bin/activate' in restart_auto, "自动盘重启脚本兼容历史venv")
        assert_true('source .venv/bin/activate' in restart_auto, "自动盘重启脚本兼容README推荐.venv")
